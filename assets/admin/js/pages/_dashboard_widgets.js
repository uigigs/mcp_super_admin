...
$("#btnSaveNotes").on('click', function () {
    ...
    $("#formnotes").submit(function (event) {
        event.preventDefault();
    });

    if (note == "") {
        $("#notesError").removeClass('hidden');

    }
    if (note != "") {
        $("#notesError").addClass('hidden');
...
$('.btn_edit_notes').on('click', function (event) {
    ...
    // var url = "{{route('admin.ip_details',"id")}}";
    var url = 'http://localhost:8000/maxadminstore/dashboard/edit-note/';
...
$("#btn_create_notes").click(function (event) {
    ...
    var ntitle = $("#edit-create-title").val();
    var ndesc = $('#edit-create-description').val();
    if (ntitle == "") {
        $("#editGetError").removeClass('hidden');
    }
    if (ntitle != "") {
...
$(".btn-delete").on('click', function (event) {
    event.preventDefault();
    var current_id = $(this).closest('td').attr('id');
    //var get=$('table#ipTable').closest('tr').attr('id');
    //var ip =$('td:first-child').text();
    // alert(ip);
    swal({
        title: "Are you sure?",
        ...
        closeOnCancel: true
            ...
            function (isConfirm)
            {
                if (isConfirm)
                {
                    var url = 'http://localhost:8000/maxadminstore/dashboard/delete-note/';
                    //url = url.replace(':id', current_id);
                    $.get(url + current_id, function (data) {
                        if (data != "") {
                            window.location.href = 'http://localhost:8000/maxadminstore/dashboard/delete-note-confirm';
                        }
                    });
